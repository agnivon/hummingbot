...
from hummingbot.connector.exchange.aster.aster_order_book import AsterOrderBook
...
class AsterAPIOrderBookDataSource(OrderBookTrackerDataSource):
    HEARTBEAT_TIME_INTERVAL = 180.0
    TRADE_STREAM_ID = 1
    DIFF_STREAM_ID = 2
    ONE_HOUR = 60 * 60
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        await ws.connect(
            ws_url=CONSTANTS.WSS_URL[self._domain], ping_timeout=CONSTANTS.WS_HEARTBEAT_TIME_INTERVAL
        )
        return ws
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = AsterOrderBook.snapshot_message_from_exchange(
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            ...
            trade_message = AsterOrderBook.trade_message_from_exchange(raw_message, {"trading_pair": trading_pair})
            message_queue.put_nowait(trade_message)

    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["s"])
            order_book_message: OrderBookMessage = AsterOrderBook.diff_message_from_exchange(
