...
from hummingbot.connector.exchange.aster import aster_constants as CONSTANTS, aster_utils, aster_web_utils as web_utils
from hummingbot.connector.exchange.aster.aster_api_order_book_data_source import AsterAPIOrderBookDataSource
from hummingbot.connector.exchange.aster.aster_api_user_stream_data_source import AsterAPIUserStreamDataSource
...
class AsterExchange(ExchangePyBase):
    ...
    def __init__(
        ...
        self.api_key = aster_api_key
        self.secret_key = aster_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_aster_timestamp = 1.0
        super().__init__(balance_asset_limit, rate_limits_share_pct)

    @staticmethod
    def aster_order_type(order_type: OrderType) -> str:
        if order_type is OrderType.LIMIT_MAKER:
            return "LIMIT"
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(aster_type: str) -> OrderType:
        return OrderType[aster_type]
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return AsterAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return AsterAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        type_str = AsterExchange.aster_order_type(order_type)
        ...
        try:
            ...
            transact_time = order_result.get("transactTime", order_result.get("updateTime", self._time_synchronizer.time() * 1e3)) * 1e-3
        except IOError as e:
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(aster_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_trading_fees(self):
        ...
        for trading_pair in self._trading_pairs:
            symbol = await self.exchange_symbol_associated_to_pair(trading_pair=trading_pair)
            tasks.append(self._api_get(path_url=CONSTANTS.COMMISSION_RATE_PATH_URL, params={"symbol": symbol}, is_auth_required=True))

        try:
            results = await safe_gather(*tasks, return_exceptions=True)
            for result, trading_pair in zip(results, self._trading_pairs):
                if isinstance(result, Exception):
                    self.logger().warning(f"Failed to fetch trading fees for {trading_pair}: {result}")
                    continue

                maker_fee = Decimal(result["makerCommissionRate"])
                taker_fee = Decimal(result["takerCommissionRate"])

                self._base_fees[trading_pair] = TradeFeeSchema(
                    maker_percent_fee_decimal=maker_fee,
                    taker_percent_fee_decimal=taker_fee,
                    buy_percent_fee_deducted_from_returns=True,
                )
        except Exception:
            self.logger().exception("Error fetching trading fees.")
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            query_time = int(self._last_trades_poll_aster_timestamp * 1e3)
            self._last_trades_poll_aster_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(aster_utils.is_exchange_information_valid, exchange_info["symbols"]):
