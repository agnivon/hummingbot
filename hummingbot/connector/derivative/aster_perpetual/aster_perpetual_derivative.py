...
class AsterPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        aster_perpetual_api_key: Optional[str] = None,
        aster_perpetual_api_secret: Optional[str] = None,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
        domain: str = CONSTANTS.DOMAIN,
    ):
        self.aster_perpetual_api_key = aster_perpetual_api_key
        self.aster_perpetual_secret_key = aster_perpetual_api_secret
    ...
    def authenticator(self) -> AsterPerpetualAuth:
        if not hasattr(self, "_auth") or self._auth is None:
            self._auth = AsterPerpetualAuth(
                api_key=self.aster_perpetual_api_key,
                api_secret=self.aster_perpetual_secret_key,
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        if cancel_result.get("code") == -2011 and "Unknown order sent." == cancel_result.get("msg", ""):
            self.logger().debug(f"The order {order_id} does not exist on Aster Perpetuals. " f"No cancellation needed.")
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        try:
            order_result = await self._api_post(path_url=CONSTANTS.ORDER_URL, data=api_params, is_auth_required=True)
        ...
        if event_type == "ORDER_TRADE_UPDATE":
            ...
            if tracked_order is not None:
                order_update: OrderUpdate = OrderUpdate(
                    trading_pair=tracked_order.trading_pair,
                    update_timestamp=order_message["T"] * 1e-3,
            ...
            for position in positions:
                ...
                if existing_position is not None:
                    existing_position.update_position(
                        position_side=PositionSide[position["ps"]],
    ...
    async def _get_last_traded_price(self, trading_pair: str) -> float:
        ...
        response = await self._api_get(path_url=CONSTANTS.TICKER_PRICE_CHANGE_URL, params=params)
    ...
    async def _api_request_url(self, path_url: str, is_auth_required: bool = False) -> str:
        if is_auth_required:
            url = self.web_utils.private_rest_url(path_url, domain=self._domain)
        else:
            url = self.web_utils.public_rest_url(path_url, domain=self._domain)
