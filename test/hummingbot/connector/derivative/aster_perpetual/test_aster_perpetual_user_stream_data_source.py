...
class AsterPerpetualUserStreamDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.connector = AsterPerpetualDerivative(
            aster_perpetual_api_key="", aster_perpetual_api_secret="", domain=self.domain, trading_pairs=[]
        )
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.derivative.aster_perpetual.aster_perpetual_user_stream_data_source.AsterPerpetualUserStreamDataSource._sleep"
    )
    async def test_get_listen_key_exception_raised(self, mock_api, _):
        url = web_utils.private_rest_url(path_url=CONSTANTS.ASTER_USER_STREAM_ENDPOINT, domain=self.domain)
    ...
    @patch(
        "hummingbot.connector.derivative.aster_perpetual.aster_perpetual_user_stream_data_source.AsterPerpetualUserStreamDataSource"
        "._ping_listen_key",
    ...
    @patch("hummingbot.connector.derivative.aster_perpetual.aster_perpetual_user_stream_data_source.safe_ensure_future")
