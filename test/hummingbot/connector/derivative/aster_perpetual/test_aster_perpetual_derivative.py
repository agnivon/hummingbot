...
class AsterPerpetualDerivativeUnitTest(IsolatedAsyncioWrapperTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.exchange = AsterPerpetualDerivative(
            aster_perpetual_api_key="testAPIKey",
            aster_perpetual_api_secret="testSecret",
        ...
        AsterPerpetualAPIOrderBookDataSource._trading_pair_symbol_map = {
    ...
    def all_symbols_url(self):
        url = web_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_INFO_URL, domain=self.domain)
        return url

    @property
    def latest_prices_url(self):
        url = web_utils.public_rest_url(path_url=CONSTANTS.TICKER_PRICE_CHANGE_URL, domain=self.domain)
        url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?") + ".*")
        return url

    @property
    def network_status_url(self):
        url = web_utils.public_rest_url(path_url=CONSTANTS.PING_URL, domain=self.domain)
        return url
    ...
    def balance_url(self):
        url = web_utils.private_rest_url(path_url=CONSTANTS.ACCOUNT_INFO_URL, domain=self.domain)
        return url
    ...
    def funding_payment_url(self):
        url = web_utils.private_rest_url(path_url=CONSTANTS.GET_INCOME_HISTORY_URL, domain=self.domain)
        url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?") + ".*")
        return url
    ...
    async def test_buy_order_fill_event_takes_fee_from_update_event(self):
        ...
        self.assertEqual(Decimal("0"), fill_event.trade_fee.percent)
        self.assertEqual(
            [TokenAmount(complete_fill["o"]["N"], Decimal(complete_fill["o"]["n"]))], fill_event.trade_fee.flat_fees
        )
    ...
    async def test_margin_call_event(self):
        ...
        mock_user_stream = AsyncMock()
        mock_user_stream.get.side_effect = functools.partial(
            self._return_calculation_and_set_done_event, lambda: margin_call
        )
        ...
        self.assertTrue(
            self._is_logged(
                "INFO", f"Margin Required: 1.614445. Negative PnL assets: {self.trading_pair}: -1.166074, ."
            )
        )
    ...
    async def test_wrong_symbol_margin_call_event(self):
        ...
        self.assertTrue(self._is_logged("INFO", "Margin Required: 0. Negative PnL assets: ."))

    @aioresponses()
    @patch(
        "hummingbot.connector.derivative.aster_perpetual.aster_perpetual_derivative."
        "AsterPerpetualDerivative.current_timestamp"
    )
    ...
    async def test_set_leverage_successful(self, req_mock):
        ...
        response = {"leverage": leverage, "maxNotionalValue": "1000000", "symbol": symbol}
    ...
    async def test_set_leverage_failed(self, req_mock):
        ...
        response = {"leverage": 0, "maxNotionalValue": "1000000", "symbol": symbol}
    ...
    async def test_cancel_all_unknown_order(self, req_mock):
        ...
        self.assertTrue(
            self._is_logged("DEBUG", "The order OID1 does not exist on Aster Perpetuals. " "No cancellation needed.")
        )
    ...
    async def test_create_order_successful(self, req_mock):
        ...
        create_response = {"updateTime": int(self.start_timestamp), "status": "NEW", "orderId": "8886774"}
    ...
    @aioresponses()
    @patch("hummingbot.connector.derivative.aster_perpetual.aster_perpetual_web_utils.get_current_server_time")
    async def test_place_order_manage_server_overloaded_error_unkown_order(
    ...
    async def test_create_order_exception(self, req_mock):
        ...
        # "Error submitting buy LIMIT order to aster_perpetual for 9999 COINALPHA-HBOT 1010."
        self.assertTrue(
            self._is_logged(
                "NETWORK",
    ...
    async def test_create_order_min_notional_size_failure(self):
        ...
        self._simulate_trading_rules_initialized()
        mocked_response = self._get_exchange_info_mock_response(
            margin_asset, min_notional_size=min_notional_size, min_base_amount_increment=0.5
        )
        trading_rules = await self.exchange._format_trading_rules(mocked_response)
        self.exchange._trading_rules[self.trading_pair] = trading_rules[0]
        trade_type = TradeType.BUY
