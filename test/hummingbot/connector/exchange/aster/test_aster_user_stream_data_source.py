...
from hummingbot.connector.exchange.aster import aster_constants as CONSTANTS, aster_web_utils as web_utils
...
from hummingbot.connector.exchange.aster.aster_exchange import AsterExchange
from hummingbot.connector.test_support.network_mocking_assistant import NetworkMockingAssistant
from hummingbot.connector.time_synchronizer import TimeSynchronizer
from hummingbot.core.api_throttler.async_throttler import AsyncThrottler


class AsterUserStreamDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.auth = AsterAuth(api_key="TEST_API_KEY", secret_key="TEST_SECRET", time_provider=self.mock_time_provider)
        self.time_synchronizer = TimeSynchronizer()
        self.time_synchronizer.add_time_offset_ms_sample(0)

        self.connector = AsterExchange(
            aster_api_key="", aster_api_secret="", trading_pairs=[], trading_required=False, domain=self.domain
        )
        self.connector._web_assistants_factory._auth = self.auth

        self.data_source = AsterAPIUserStreamDataSource(
            auth=self.auth,
    ...
    @aioresponses()
    @patch("hummingbot.connector.exchange.aster.aster_api_user_stream_data_source.AsterAPIUserStreamDataSource._sleep")
    async def test_get_listen_key_log_exception(self, mock_api, _):
        url = web_utils.private_rest_url(
            path_url=CONSTANTS.ASTER_USER_STREAM_PATH_URL,
        )
    ...
    @patch(
        "hummingbot.connector.exchange.aster.aster_api_user_stream_data_source.AsterAPIUserStreamDataSource"
        "._ping_listen_key",
    ...
    async def test_manage_listen_key_task_loop_keep_alive_failed(self, mock_ping_listen_key):
        ...
        self.assertTrue(
            self._is_logged("ERROR", "Error occurred renewing listen key ... No active exception to reraise")
        )
    ...
    async def test_listen_for_user_stream_get_listen_key_successful_with_user_update_event(self, mock_api, mock_ws):
        ...
        expected_ws_url = f"{CONSTANTS.USER_STREAM_WSS_URL[self.domain]}?listenKey={self.listen_key}"
        self.assertEqual(expected_ws_url, mock_ws.call_args[1].get("url") or mock_ws.call_args[0][0])
    ...
    @patch("hummingbot.connector.exchange.aster.aster_api_user_stream_data_source.safe_ensure_future")
